...
static void chip8_exec_extended_eight(struct chip8* chip8, unsigned short opcode)
{
  ...
  switch(final_four_bits)
  {
    ...
    // 8xy1 - AND Vx, Vy. Performs a bitwise AND on Vx and Vy stores the result in Vx
    case 0x02:
      chip8->registers.V[x] = chip8->registers.V[x] & chip8->registers.V[y];
    break;

    // 8xy1 - XOR Vx, Vy. Performs a bitwise XOR on Vx and Vy stores the result in Vx
    case 0x03:
      chip8->registers.V[x] = chip8->registers.V[x] ^ chip8->registers.V[y];
    break;
...
static void chip8_exec_extended(struct chip8* chip8, unsigned short opcode)
{
  ...
  switch (opcode & 0xf000)
  {
    ...
    // SNE Vx, byte - 4xkk Skip the next instruction if Vx!=kk
    case 0x4000:
      if (chip8->registers.V[x] != kk)
      {
        chip8->registers.PC += 2;
    ...
    // 5xy0 - SE Vx, Vy - Skip the next instruct if Vx=Vy
    case 0x5000:
      if (chip8->registers.V[x] == chip8->registers.V[y])
      {
        chip8->registers.PC += 2;
